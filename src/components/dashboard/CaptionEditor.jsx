...
export default function CaptionEditor({
  ...
  useEffect(() => {
    // Show tip only once per user
    if (!user?.id) return;
    const key = `base44_caption_pro_tip_shown_${user.id}`;
    const hasSeenTip = localStorage.getItem(key);
    if (!hasSeenTip) {
      setShowAutoTip(true);
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
        <AnimatePresence>
          {captions?.filter(cap => cap && cap.id).map((caption, index) => (
            <motion.div
              key={caption.id}
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-3">
                      {wordPopup && wordPopup.caption.id === caption.id && (
                      ...
                      <div className="mb-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(caption.text || '').split(/\s+/).map((word, wordIdx) => {
                            const isSelected = wordPopup && wordPopup.caption.id === caption.id && wordPopup.wordIndex === wordIdx;
                            return (
                              <button
                                key={wordIdx}
                                onClick={(e) => {
